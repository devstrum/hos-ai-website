...
const PARTNERS = [
  { name: 'Ringg', domain: 'ringg.ai', tag: 'PREFERRED · VOICE AI · UAE', desc: 'Our default stack for any conversational deployment - inbound, outbound, IVR replacement.' },
    ...
    <div style={{ background: bg, color: ink, fontFamily: '"Geist Mono", monospace', minHeight: '100%' }} data-screen-label="About">
      ...
      <Reveal as="section" style={{ padding: `clamp(48px, 10vw, 96px) ${PAD_X}`, borderTop: `1px solid ${ink}`, textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 24, maxWidth: 640, marginLeft: 'auto', marginRight: 'auto' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: muted, letterSpacing: '.1em' }}>FOUNDER, HOUSE OF SHAFAQ</div>
          </div>
          <p style={{ fontFamily: 'Inter, sans-serif', fontSize: 16, lineHeight: 1.65, color: 'rgba(14,26,43,.8)', margin: 0 }}>
          ...
          <div>
            <span style={{ display: 'inline-block', width: 24, height: 2, background: accent, marginBottom: 16 }} />
      ...
      <Reveal as="section" style={{ padding: `clamp(48px, 10vw, 96px) ${PAD_X}`, borderTop: `1px solid ${ink}`, textAlign: 'center' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 1, background: rule, border: `1px solid ${rule}` }}>
          {PARTNERS.map((p) => (
            <div key={p.name} style={{ background: bg, padding: 28, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
